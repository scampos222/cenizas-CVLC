...
# ==========================================
# 3. MOTOR DE DATOS CACHEADO (SEGURO)
# ==========================================
...
def cargar_y_limpiar_datos(archivo, url_gs):
    ...
    if df_temp is None:
        datos_prueba = {
            ...
            'Tamaño_Promedio_mm': [0.5, 0.8, 2.1, 1.5], 'Espesor_Deposito_mm': [10, 5, 2, 8],
...
def renderizar_modulo_espacial(df_fil, archivo_geo):
    try:
        st.subheader("Modelamiento y Distribución Cartográfica")
        tipo_mapa = st.radio(
            "Seleccione la capa espacial:", 
            ["📍 Puntos (2D)", "🔥 Calor (2D)", "🌋 Vista 3D (Volumen)", "🎯 Isopacas (Espesor interpolado)", "🪨 Isopletas (Tamaño interpolado)"], 
            horizontal=True
        )
        st.markdown("---")
        ...
        # RENDERIZADO 2D (FOLIUM Y MATPLOTLIB)
        else:
            ...
            if "Puntos" in tipo_mapa:
                ...
                for _, r in df_mapa.iterrows():
                    html = f"<div style='font-family:sans-serif;'><b>{r.get('ID_Muestra', 'N/A')}</b><br>Vereda: {r.get('Vereda', 'N/A')}<br>Espesor: {r.get('Espesor_Deposito_mm', 0)} mm<br>Tamaño: {r.get('Tamaño_Promedio_mm', 0)} mm</div>"
                ...
                # Agrupamos por coordenada exacta (para evitar error matricial), asumiendo el caso más crítico (max)
                df_mod = df_mod.groupby(['Latitud', 'Longitud'], as_index=False).agg({col_obj: 'max', 'ID_Muestra': 'first'})
                
                if len(df_mod) < 4:
                    st.warning(f"⚠️ Se requieren al menos 4 puntos para la interpolación matemática de {col_obj}.")
                else:
                    lon, lat, z = df_mod['Longitud'].values, df_mod['Latitud'].values, df_mod[col_obj].values
                    margen_lon = (lon.max() - lon.min()) * 0.2 if lon.max() != lon.min() else 0.05
                    margen_lat = (lat.max() - lat.min()) * 0.2 if lat.max() != lat.min() else 0.05
                    ...
                    # Generación de la imagen transparente con Matplotlib
                    fig = plt.figure(frameon=False)
                    ax = fig.add_axes([0, 0, 1, 1])
                    ax.axis('off')
                    ax.set_xlim(lim_lon_min, lim_lon_max)
                    ax.set_ylim(lim_lat_min, lim_lat_max)
                    cmap_choice = 'Reds' if "Isopacas" in tipo_mapa else 'viridis'
                    
                    contour = ax.contourf(grid_lon, grid_lat, grid_z, alpha=0.6, cmap=cmap_choice, levels=12)
                    ax.contour(grid_lon, grid_lat, grid_z, colors='black', linewidths=0.6, levels=12) # Líneas fuertes
                    ...
                    # Superponer la imagen en Folium
                    folium.raster_layers.ImageOverlay(
                        image=img_url,
                        bounds=[[lim_lat_min, lim_lon_min], [lim_lat_max, lim_lon_max]],
                        opacity=0.75,
                        name=f"Modelo de {col_obj}"
                    ).add_to(m)
                    
                    # Añadir los puntos reales encima
                    for _, r in df_mod.iterrows():
                        folium.CircleMarker([r['Latitud'], r['Longitud']], radius=3, color="black", fill=True, popup=f"{r['ID_Muestra']}: {r[col_obj]}").add_to(m)

            folium.LayerControl().add_to(m)
            st_folium(m, width="100%", height=600)
            if "Isopacas" in tipo_mapa or "Isopletas" in tipo_mapa:
                st.caption("*Nota: La interpolación geomatemática en los bordes del área muestreada presenta mayor incertidumbre.*")
    ...
    try:
        ...
        with col_f:
            ...
            if fotos_locales:
                ...
                if min_clic:
                    for i, f_obj in enumerate(fotos_locales):
                ...
                else: st.info("No hay fotos asignadas a esta muestra.")
            else: st.info("No hay fotos asignadas a esta muestra.")

        st.markdown("---")
        
        # --- 2. ANÁLISIS COMPARATIVO Y TERNARIO ---
        st.subheader("2. Comparativa y Petrología General")
        c1, c2 = st.columns(2)
        ...
        if not df_tp.empty:
            ...
            fig_t = px.scatter_ternary(df_tp, a='V%', b='L%', c='C%', color="Vereda", hover_name="ID_Muestra", size="Tamaño_Promedio_mm", color_discrete_sequence=colores_profesionales, title="Clasificación Petrológica (V-L-C)")
        ...
        with col_t1:
            ...
            else: st.info("Requiere datos temporales para evolución mineralógica.")
...
with st.sidebar.expander("📂 Carga de Datos", expanded=True):
    ...
    st.markdown("---")
    st.markdown("<small><i>Sube varias fotos locales a la vez (El ID de la muestra debe estar en el nombre del archivo).</i></small>", unsafe_allow_html=True)
...
if df_fil.empty: st.warning("⚠️ Sin resultados para los filtros aplicados.")
    ...
    # ESTRUCTURA DE 3 MACRO-PESTAÑAS
    t_espacial, t_laboratorio, t_operativo = st.tabs(["🌍 Módulo Espacial (Mapas y Modelos)", "🔬 Módulo de Laboratorio (Mineralogía y Tamaño)", "🗃️ Módulo Operativo (Datos y Reportes)"])
